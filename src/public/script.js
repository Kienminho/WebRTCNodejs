...
$("#joinExistingMeeting").on("click", async function (e) {
  ...
  // gửi request đến validate meeting id
  try {
...
$("#createANewMeeting").on("click", async function (e) {
  ...
  // gửi request để tạo mới meeting room
  try {
...
meeting.on("activeSpeaker", function (activeSpeaker) {
  ...
  if ($(`#participant-${activeSpeaker.participantSessionId}-video`)) {
    let stream = $(
  ...
  console.log("acc:" + activeSpeaker.participantSessionId);
  console.log("ss:" + meeting.participantSessionId);
